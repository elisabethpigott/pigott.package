...
The other two functions, richness and Simpson, can also be analyzed, but we will not be executing this code during the presentation. I will drop the code below if you want to look at the richness and Simpson plots yourself.
...
This function uses metaMDS to perform an NMDS ordination on a community dataset. This function will show me if my sites are similar/dissimilar from one another by which species I caught at each site. The input must be a data frame with the community data spread out, aka species as columns and sites as rows.

First, we need to transpose my original matrix so that species are in columns and sites are in rows. For whatever reason, metaMDS and iNEXT have opposite input matrix formats.
```{r}
